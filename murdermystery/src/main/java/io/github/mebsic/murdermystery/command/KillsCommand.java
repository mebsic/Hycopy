...
public class KillsCommand implements CommandExecutor {
    ...
    public KillsCommand(CorePlugin corePlugin) {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 2) {
            ...
            sender.sendMessage(ChatColor.RED + "/kills <player> <amount>");
            return true;
        }
        ...
        try {
            ...
            sender.sendMessage(ChatColor.RED + "Kills amount must be a number!");
            return true;
        }
        if (amount < 0) {
            sender.sendMessage(ChatColor.RED + "Kills amount must be 0 or higher!");
            return true;
        }
        ...
        if (sync != null) {
            sync.dispatchCounterSet(
                    uuid,
                    MongoManager.MURDER_MYSTERY_LIFETIME_KILLS_KEY,
                    amount,
                    null,
                    null
            );
    ...
    private void setKills(Profile profile, int amount) {
        if (profile == null || profile.getStats() == null) {
            return;
        }
        int current = Math.max(0, profile.getStats().getKills());
        if (current != amount) {
            profile.getStats().addKills(amount - current);
        }
    }
}
